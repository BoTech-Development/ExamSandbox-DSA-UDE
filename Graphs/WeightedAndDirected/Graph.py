...
class Graph:
    class Bag:
        ...
        def remove(self, edgeToRemove : Edge) -> None:
            ...
            while currentNode is not None:
                if currentNode.value.target() == edgeToRemove.target(): # wichtig ist am schlausten den index des anderen Knotens zu vergleichen, da man sonst immer das gleiche Objekt ( reference) in die Methode stopfen muss.
    ...
    def addEdge(self, edgeToAdd : Edge) -> None:
        self.__adjacencyList[edgeToAdd.source()].add(edgeToAdd)

    def removeEdge(self, edgeToRemove : Edge) -> None:
        self.__adjacencyList[edgeToRemove.source()].remove(edgeToRemove)
